...
var projectArray:Project[] = [
  ...
  },
  {
    ...
    "fg_max": 4,
    "repeatRules": [
      ["repeat"], ["no-repeat"], ["repeat-x"], ["repeat-x"], ["repeat-x"]
    ]
  }
]
...
function ParallaxElements() {
  ...
  useEffect(() => {
    ...
    const scrollContainer = (gapTime: number) => {
      ...
      if (loops >= 3) { // 0.333x the FPS (60fps => 20fps)
        container.scrollLeft += Math.max((0.1 * dt) / window.devicePixelRatio, 1);
        loops = 0;
      }
    ...
    childrenMush.forEach((scrollBox) => {
      rects.push(Array.from(scrollBox.children as HTMLCollectionOf<HTMLElement>).map(child => +(child.dataset?.depth ?? 0)));
    });
    const parallaxContainer = () => {
      childrenMush.forEach((scrollBox, i) => {
        ...
        var relativeMouseX = (mouseX / scrollBox.clientWidth) - (getRekt.left / scrollBox.clientWidth);
        var relativeMouseY = (mouseY / scrollBox.clientHeight) - (getRekt.top / scrollBox.clientHeight);
        Array.from(scrollBox.children as HTMLCollectionOf<HTMLElement>).forEach((parallaxElement, ii) => {
          ...
          parallaxElement.style.transform = `translate(${Math.floor(((progressX * scrollBox.clientWidth) * (myDepth * 1.5)) + mouseEffectX)}px, ${Math.floor(((progressY * scrollBox.clientHeight) * (myDepth * 1.5)) + mouseEffectY)}px)`;
        });
      });
  ...
  var elements = projectArray.map((v, i) => <div key={i} className="coverImage"
  style={{
    backgroundColor: generateRandomColor()
  }}>
    <div className="prlx effectImage heroImage" data-depth="-0.6"
      style={{
        backgroundImage: `url("${v.dir}bg.png")`
      }}
    ></div>
    {v.repeatRules.map((canRepeat, i) => 
      <div className="prlx effectImage" data-depth={canRepeat[1] ?? ((0 - v.fg_max) + (i - 1)) / 10}
        style={{
    ...
    <div className="prlx titleImage" data-depth="-0.1"
      style={{
